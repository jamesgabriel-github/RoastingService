...
**Resolution:** Re-examined 2026-09-25 by /audit independent (Feature 12 review of `fddfa20`; scope: current; all lenses). Feature 12 did not change `approve`. The assignment is at `BookingController.php:94` and still stores the raw validated string. The new timestamps (`cooking_started_at`, `est_ready_at`, `completed_at`) come from `now()`, not from client input, so they are not affected. Still open. Re-examined 2026-09-26 by /audit independent (Feature 14 review; scope: current; all lenses). Feature 14 touched `BookingController.php` only to make `detailEagerLoads()` public and add `'payments'` to it; `approve()` and its `dropoff_at` assignment at line 94 are unchanged. Still open.
...
### F-60 [P2] open - paid_amount/balance are computed with float arithmetic on decimal money columns

**File:** backend/app/Http/Resources/AdminBookingResource.php:21-23,39
**Found:** 2026-09-26 by /audit independent (scope: current; lens: quality)
**Why it matters:** `amount`/`total_amount` are `decimal:2`-cast Eloquent attributes (PHP strings, e.g. `"705.00"`). `$this->payments->where('status', 'paid')->sum('amount')` sums those strings with PHP's `+` operator, which coerces them to native float, and the surrounding `(float)` casts on lines 22 and 39 make that explicit for both the sum and the `total_amount - paid_amount` subtraction. `coding-standards.md`'s Database section says "Money and other precise decimals: use decimal/numeric columns, never float," and this feature's own spec (`current-feature.md`, Notes for the AI) repeats it for this exact code: "keep amounts as decimal/numeric strings end-to-end - never float." Both are violated in the one place this feature adds money math. For a `decimal(10,2)` column capped at 99,999,999.99, and with at most one `paid` payment per booking (the double-payment guard in `BookingPaymentController::store`), the values stay well inside float64's exact-integer range and `number_format(..., 2)` rounds away any binary epsilon, so no incorrect amount is reachable today. The pattern is still a direct, explicit contract violation by construction, and would become a real precision risk the moment a second summed payment (the roadmapped `downpayment`/`balance` types) needs to be added on top of it.
**Suggested fix:** Do the sum and subtraction as strings, for example `bcsub($this->total_amount, $paidAmount, 2)` for the balance, and format the payments sum from a query-side numeric string rather than an intermediate `(float)` cast. Current requirement lost: None.
**Resolution:**

### F-61 [P3] open - Payments list page does not surface query errors

**File:** frontend/src/features/admin-payments/hooks.ts:5; frontend/src/features/admin-payments/PaymentsListPage.tsx:14
**Found:** 2026-09-26 by /audit independent (scope: current; lens: quality)
...
**Resolution:** Re-examined 2026-09-26 by /audit independent (fix/booking-type-is-order review of `60cd762`; scope: current; all lenses). `salesSince()` is now keyed by `bookings.is_order::int` instead of `bookings.source_type`, shifting the two `(float)` casts to `DashboardController.php:59-60`; the float-arithmetic pattern itself is unchanged and unaffected by this fix. Still open.
...
**Resolution:** Re-examined 2026-09-26 by /audit independent (fix/booking-type-is-order review of `60cd762`; scope: current; all lenses). This fix did not touch `DashboardPage.tsx`'s loading/error handling or `hooks.ts`; only the `SalesCard` labels and `SalesPeriod` field names changed (`not_order`/`is_order`). Still open.

### F-64 [P3] open - preferred_pickup_at repeats F-51's unnormalized-timestamp assignment in four new create paths

**File:** backend/app/Http/Controllers/Api/Customer/BookingController.php:114; backend/app/Http/Controllers/Api/Customer/OrderController.php:78; backend/app/Http/Controllers/Api/Admin/WalkInController.php:87,176
**Found:** 2026-09-26 by /audit independent (scope: current; lens: quality/security)
**Why it matters:** All four new `preferred_pickup_at` assignments pass the raw validated request string straight into the `datetime`-cast `Booking::$preferred_pickup_at` attribute (`'preferred_pickup_at' => $request->validated('preferred_pickup_at')`), the same as `preferred_dropoff_at` already does. F-51 documents that Eloquent's `datetime` cast keeps a value's original UTC offset rather than converting to UTC before storing in the zoneless `timestamp` column (app timezone `UTC`), so a caller who sends an offset other than `Z` (for example `"2026-10-01T10:00:00+08:00"`) has it stored 8 hours later than intended, even though it still passes the `after`/`after_or_equal` validation correctly. This fix extends that same pattern to four brand-new call sites: `Customer\BookingController::store` (bring-your-own), `Customer\OrderController::store` (shop order), and `Admin\WalkInController::storeRoasting`/`storeShop`. F-51 already flags one of these methods (`Customer\BookingController::store`) as exhibiting the pattern for `preferred_dropoff_at`; this diff adds a second raw-offset-preserving field in that same array literal, and spreads the identical pattern into three more controllers. The shipped SPA always sends `.toISOString()` (`Z`) values for every new pickup field, so the UI path is unaffected; only a direct API caller using a non-`Z` offset is affected, matching F-51's existing risk profile.
**Suggested fix:** Apply F-51's fix (normalize with `Carbon::parse(...)->utc()` before assignment) to `preferred_dropoff_at` and `preferred_pickup_at` together across all five affected sites (the pre-existing `dropoff_at`/`preferred_dropoff_at` cases plus these four new ones), ideally through one shared helper so a future date field does not repeat the gap a sixth time. Current requirement lost: None.
**Resolution:**

### F-65 [P3] open - No test covers an invalid `date` query param on the admin bookings list

**File:** backend/app/Http/Controllers/Api/Admin/BookingController.php:53; backend/tests/Feature/Admin/AdminBookingQueuesTest.php:99
**Found:** 2026-09-26 by /audit independent (scope: current; lens: tests)
**Why it matters:** `index()`'s new `'date' => ['nullable', 'date_format:Y-m-d']` rule should turn a malformed value (for example `date=not-a-date` or a `d/m/Y` value) into a 422, but no test exercises that path. The only new test, `test_date_filter_excludes_bookings_outside_the_selected_date`, sends a well-formed `Y-m-d` value. The default-to-today path (no `date` param) is exercised only incidentally by pre-existing tests such as `test_status_filter_returns_only_that_queue` and `test_waiting_minutes_is_a_non_negative_whole_number`, which now pass only because `BookingFactory` was updated to default `preferred_pickup_at` to `now()`; neither test asserts the default-date behavior directly. The rule reads correctly, so this is a coverage gap, not a known defect.
**Suggested fix:** Add a test that requests `/api/v1/admin/bookings?status=...&date=not-a-date` and asserts `422` with a validation error on `date`. Optionally add an explicit assertion that omitting `date` returns only bookings scheduled today. Current requirement lost: None.
**Resolution:**

### F-70 [P3] open - No backend test covers whole-booking actions or customer status on a booking whose items have diverged

**File:** backend/app/Models/Booking.php:86; backend/app/Http/Controllers/Api/Customer/BookingController.php:50; backend/app/Http/Controllers/Api/Admin/BookingController.php:311-349; backend/app/Http/Resources/BookingResource.php:25
**Found:** 2026-09-27 by /audit independent (scope: current; lens: tests)
**Why it matters:** The feature's central new invariant is that once a booking's items diverge (one `cooking`, a sibling `confirmed`), `Booking::commonStatus()` returns `null` and every whole-booking action (admin approve/reject/weigh-in/confirm-order/reject-order/no-show/cancel, customer cancel) is refused, while the customer `BookingResource.status` becomes `null`. Frontend tests cover the `null`-on-divergence rule for `isCancellable`/`isAdminCancellable`/`getCommonStatus`, but no backend test seeds a diverged booking: `test_two_items_on_the_same_booking_can_be_at_different_fulfillment_stages` only exercises item routes, and every whole-booking and customer test uses items that share one status. A regression in `commonStatus()` (for example returning the first item's status) would let an admin or customer cancel a booking with a `confirmed` first item and a `cooking` sibling; the engine would then throw `InvalidArgumentException` for the cooking item, turning a clean 422 into a 500. The code reads correctly, so this is a coverage gap, not a known defect.
**Suggested fix:** Add one admin test (cancel on a `confirmed`+`cooking` booking returns 422 and writes no log) and one customer test (the same booking's `GET /bookings/{id}` returns `status: null`, and `POST /bookings/{id}/cancel` returns 422). Current requirement lost: None.
**Resolution:**

### F-71 [P3] open - Walk-in tests send `preferred_pickup_at = now()` against an `after_or_equal:now` rule and fail when the second rolls over

**File:** backend/tests/Feature/Admin/AdminWalkInBookingTest.php:61,97,122,170,191; backend/app/Http/Requests/Admin/StoreWalkInRoastingRequest.php:47; backend/app/Http/Requests/Admin/StoreWalkInShopOrderRequest.php:47
**Found:** 2026-09-27 by /audit independent (scope: current; lens: tests)
**Why it matters:** Five walk-in tests post `'preferred_pickup_at' => now()->toIso8601String()` (second precision), computed before the admin login and request, while the rule compares against a fresh `now` at validation. When the wall clock crosses a second boundary in between, the value is one second in the past and the request gets a 422. This was observed during this review: the first `composer test` run failed `test_roasting_walk_in_for_a_registered_customer_succeeds` with "The preferred pickup at field must be a date after or equal to now.", and an immediate rerun passed 259/259. The lines predate this feature (added in `e2f3b1e`), but the file is part of this delta and the flake makes the suite's green result unreliable.
**Suggested fix:** Freeze time in these tests (`$this->freezeTime()` or `Carbon::setTestNow()`), or send a value safely in the future such as `now()->addHour()->toIso8601String()`. Current requirement lost: None.
**Resolution:**

### F-72 [P3] open - New Completed and Cancelled tab counts are all-time while the list under them is date-scoped

**File:** backend/app/Http/Controllers/Api/Admin/BookingController.php:43-53,63-66; frontend/src/features/admin-bookings/AdminBookingsPage.tsx:51
